...
namespace CourseWork
{
    ...
    {
        public Form7(ref RedBlackTree<DateKey, ProcessedReq> tree)
        {
            copy = tree;
            InitializeComponent();
        }
        RedBlackTree<DateKey, ProcessedReq> copy;
        ...
        public ProcessedReq NewProcRequest()
        {
                var date = textBox1.Text;
                string[] s = date.Split('.');
                DateKey Key = new DateKey(int.Parse(s[0]), int.Parse(s[1]), int.Parse(s[2]));
